...
1. **qnl-deploy user** - Minimal permissions to:
   - Create/update/delete CloudFormation stacks (scoped to qnl-* stacks only)
   - Upload templates to the qnl-deploy-templates S3 bucket
   - Pass the CloudFormation service role
   - Assume app-specific deploy roles (created by CloudFormation)

2. **qnl-cloudformation-role** - CloudFormation execution role with permissions to:
...
```bash
aws s3 mb s3://qnl-deploy-templates --region ap-southeast-2
```

Or via AWS Console:
1. Go to S3 > Create bucket
2. Bucket name: qnl-deploy-templates
...
5. Role name: qnl-cloudformation-role
6. Create role

### Step 3: Update qnl-deploy user policy

1. Go to IAM > Users > qnl-deploy
2. Remove any existing inline policies
3. Create a new inline policy with the contents of `qnl-deploy-minimal.json`

## How It Works

1. **Template upload**: Deploy script uploads CFN templates to qnl-deploy-templates bucket
2. **Stack deployment**: CloudFormation creates/updates stack using qnl-cloudformation-role
