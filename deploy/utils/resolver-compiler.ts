  ...
  public async compileAndUploadResolvers(): Promise<string> {
    ...
    for (const relPath of compiledFilesRelative) {
      ...
      try {
        ...
        logger.success(`✓ Uploaded resolver ${relPath} to s3://${this.s3BucketName}/${s3KeyHashed}`);
      } catch (err: unknown) {
        const errorMessage = err instanceof Error ? err.message : String(err);
    ...
    logger.success(`   ✓ Successfully compiled: ${compiledFilesRelative.length} resolvers`);

    if (failedResolvers.length > 0) {
      logger.error(`   ✗ Failed to compile: ${failedResolvers.length} resolvers`);
