...
import { ANSWER_TIMEOUT_MS } from '@quiz/shared';
import type {
  PlayerJoinedPayload,
  PlayerLeftPayload,
...
interface GameEventHandlers {
  ...
  setupUserSubscriptions: (channel: Ably.RealtimeChannel, onKicked: () => void) => () => void;
...
export function useGameEventHandlers(): GameEventHandlers {
  ...
  const addCompletedQuestion = useGameStore((state) => state.addCompletedQuestion);
  const clearCompletedQuestions = useGameStore((state) => state.clearCompletedQuestions);
  const setEarnedBadgesThisQuestion = useGameStore((state) => state.setEarnedBadgesThisQuestion);
   ...
   */
  const setupRoomSubscriptions = useCallback((channel: Ably.RealtimeChannel) => {
    // Player events
    ...
    channel.subscribe('question_start', (message) => {
      ...
      // First question shows countdown
      if (payload.questionIndex === 0) {
        setBuzzerState(false, null, null, null);
    ...
    channel.subscribe('answer_result', (message) => {
      ...
      setAnswerState(payload.answerIndex, payload.correctIndex, payload.isCorrect);
    });
    ...
    channel.subscribe('question_end', (message) => {
      ...
      if (question || payload.questionText) {
        addCompletedQuestion({
          question: payload.questionText || question?.text || 'Unknown question',
   ...
   */
  const setupPresenceSubscriptions = useCallback((channel: Ably.RealtimeChannel) => {
