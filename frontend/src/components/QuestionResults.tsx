...
export function QuestionResults({
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      const remaining = Math.max(0, Math.ceil((nextQuestionTime - Date.now()) / 1000));
        ...
        {wasAnswered ? (
          wasCorrect ? (
            ...
            <div className="text-4xl font-bold text-red-400 mb-2">
              {buzzerWinnerName ? `${buzzerWinnerName} got it wrong!` : 'Incorrect!'}
            </div>
          )
        ) : (
      ...
      >
        <Card className="bg-gray-800/50">
          <CardBody className="p-4">
            ...
            <Progress
              value={nextQuestionTime ? ((nextQuestionTime - Date.now()) / 12000) * 100 : 0}
              color="primary"
              size="sm"
              className="mb-4"
              aria-label="Time until next question"
            />
            ...
            <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className={`font-semibold ${isCurrentPlayer ? 'text-primary-300' : 'text-white'}`}>
                        {entry.displayName}
              ...
              {!leaderboard.slice(0, 5).some(e => e.userId === currentPlayerId) && (
                <div className="border-t border-gray-700 pt-2 mt-2">
                  {leaderboard.filter(e => e.userId === currentPlayerId).map((entry) => (
                    <div
                      key={entry.userId}
